...
const props = defineProps<{
  /// 目标路径规则 ID（传给 Panel）
  ruleId: number
  /// 目标路径（用于 header 显示，区分同时存在的多个 slideover）
  rulePath: string
}>()
...
const panelRef = ref<InstanceType<typeof SitePathCheckHistoryPanel> | null>(null)
...
<template>
  <USlideover
    v-model:open="open"
    :ui="{
      /// 5 列 UTable + 错误信息展示需要更宽；与 SiteCheckHistorySlideover 取值一致
      content: 'max-w-7xl',
  ...
  >
    <template #header>
      <h2 class="text-base font-medium">子路由探测历史 — {{ props.rulePath }}</h2>
      <!--
        右侧：刷新按钮 + 最后刷新时间 + 关闭按钮。
    ...
    <template #body>
      <!--
        ...
        用 v-if 让 panel 仅在 slideover 打开时挂载，避免一次性渲染多个 panel。
      -->
      <SitePathCheckHistoryPanel
        v-if="open"
        ref="panelRef"
        :rule-id="props.ruleId"
      />
    </template>
